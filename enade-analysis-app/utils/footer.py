...
def show_footer(
    ...
    if advisor_text:
        if advisor_link:
            advisor_section = f'<div>{advisor_text} • <a href="{advisor_link[1]}" target="_blank" style="color:{text_color}; text-decoration:underline;">{advisor_link[0]}</a></div>'
        else:
            advisor_section = f'<div>{advisor_text}</div>'
    else:
        advisor_section = ""
